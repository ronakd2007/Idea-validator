...
/**
 * The founder's idea dashboard, rendered for two audiences.
 *
 * Pass ideaId for the owner's own view; pass publicId to render the very same
 * page for someone holding a share link. A shared report is meant to show what
 * the founder sees, so this is one component rather than two that drift apart.
 *
 * In public mode every owner-only ACTION is removed (re-validate, share,
 * export, AI regeneration, startup listing, review rating, AI chat) and the
 * server sends a payload with reviewer contact details already stripped.
 */
const TAB_SETTING: Record<string, string> = {
  scores: 'showScores',
  surveys: 'showSurveys',
  insights: 'showInsights',
  experts: 'showExpertComments',
};

export default function IdeaDashboardView({ ideaId, publicId }: { ideaId?: string; publicId?: string }) {
  ...
  useEffect(() => {
    // Public link: one unauthenticated request returns everything the owner's
    // several authenticated calls assemble.
    if (isPublic) {
      const fromHashPublic = window.location.hash.replace('#', '');
      if (TABS.some((t) => t.id === fromHashPublic)) setTab(fromHashPublic as TabId);
      api.getPublicIdeaReport(publicId as string)
        .then((d: any) => {
          const st = d.settings || {};
          setShareSettings(st);
          ...
          setData({ idea: d.idea, aggregated: d.aggregated || {} });
          setBenchmark(d.benchmark ?? null);
          setSurveys(d.surveys || []);
          setSurveyAnalytics(d.surveyAnalytics ?? null);
        ...
        .finally(() => { setLoading(false); setSurveysLoaded(true); });
      return;
    }
    ...
    api.getIdeaBenchmark(ideaId as string).then(setBenchmark).catch(() => {});
    // 403 here simply means the idea isn't validated yet — not an error state.
    api.getStartupForIdea(ideaId as string).then((r: any) => setStartup(r.startup)).catch(() => {});
    api.getIdeaDashboard(ideaId as string)
      .then((d) => {
        setData(d);
  ...
  const generateAiSummary = async (refresh = false) => {
    ...
    setAiError('');
    try {
      const res = await api.getAiSummary(ideaId as string, refresh);
  ...
  return (
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-10 flex-1 min-w-0">
      ...
      <div className="mb-6">
        <div className="flex flex-wrap items-start justify-between gap-x-6 gap-y-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {!isPublic && primaryAction?.kind === 'survey' && (
      ...
      <div className="border-b border-slate-200 mb-6 -mx-4 px-4 sm:mx-0 sm:px-0">
        <div className="flex gap-1 overflow-x-auto whitespace-nowrap -mb-px">
          {TABS.filter((t) => visibleTab(t.id)).map((t) => (
            <button
              key={t.id}
      ...
      {/* ---------- Surveys ---------- */}
      {tab === 'surveys' && can('showSurveys') && surveysBlock}

      {/* ---------- Insights ---------- */}
      {tab === 'insights' && can('showInsights') && (
        <>
          {gapFinding ? (
            <ValidationGapCard finding={gapFinding} ideaId={isPublic ? undefined : idea?.id} />
          ) : (
            <div className="bg-white border border-slate-200 shadow-sm rounded-xl p-6 mb-6">
      ...
      {/* ---------- Experts & Feedback ---------- */}
      {tab === 'experts' && can('showExpertComments') && (
        !expertDone ? (
          <EmptyState
            icon="💬"
          ...
          <>
                    ...
                    <div key={i} className="border border-slate-200 rounded-lg p-4">
                      <div className="flex items-center justify-between flex-wrap gap-2 mb-3 pb-3 border-b border-slate-100">
                        ...
                        <div className="flex items-center gap-3 text-xs flex-wrap">
                          {!isPublic && fb.validatorEmail && <a href={`mailto:${fb.validatorEmail}`} className="text-blue-600 hover:underline">{fb.validatorEmail}</a>}
                          {!isPublic && fb.validatorPhone && <a href={`tel:${fb.validatorPhone}`} className="text-blue-600 hover:underline">{fb.validatorPhone}</a>}
                          {!isPublic && fb.validatorLinkedinUrl && <a href={fb.validatorLinkedinUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">LinkedIn</a>}
                        </div>
                      </div>
            ...
            {/* Interested Contacts */}
            {!isPublic && a.interestedContacts?.length > 0 && (
    ...
    {!isPublic && idea && <AIChatPanel targetType="idea" targetId={idea.id} readOnly={viewMode} />}
    </div>
  );
}
