...
import { windowActivation, lerp } from '@/components/landing/utils';
import { VALIDATOR_STAGES, TEXT_FADE, type ValidatorStageText } from './validatorSceneConfig';

const SIDE_CLASSES: Record<ValidatorStageText['side'], string> = {
  center: 'items-center justify-center text-center left-0 right-0',
  left: 'items-center justify-start text-left left-0 w-full md:w-[46%] xl:w-[40%] pl-6 md:pl-16',
  right: 'items-center justify-end text-right left-0 w-full md:w-[46%] xl:w-[40%] md:ml-auto pr-6 md:pr-16',
};

function TextBlock({ stage, activation }: { stage: ValidatorStageText; activation: number }) {
  const translateY = lerp(16, 0, activation);
  const needsScrim = stage.side === 'center';
...
/**
 * DOM copy overlay — the founder landing's SceneText pattern verbatim:
 * only the single most-active stage is ever mounted (each block is a fixed
 * inset-0 layer; two at once is two headlines in the same box), and a fixed
 * scrim guarantees contrast regardless of what the 3D scene is doing.
 */
export default function ValidatorSceneText() {
  const progress = useScrollProgress();

  let winner: { stage: ValidatorStageText; activation: number } | null = null;
  for (const stage of VALIDATOR_STAGES) {
