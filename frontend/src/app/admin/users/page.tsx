      ...
      <div className="flex gap-2 mb-6">
        {['', 'FOUNDER', 'VALIDATOR'].map(r => (
          <button key={r} onClick={() => setFilter(r)}
            className={`px-4 py-1.5 rounded-lg text-sm font-medium transition ${filter === r ? 'bg-indigo-600 text-white' : 'bg-white border border-gray-300 text-gray-600 hover:border-indigo-400'}`}>
            {r || 'All'}
          </button>
        ))}
      </div>
              ...
              <tr key={u.id} className="border-b border-gray-100 hover:bg-gray-50">
                ...
                <td className="px-6 py-4">
                  <button onClick={() => toggleStatus(u.id)} disabled={actionLoading === u.id}
                    className={`text-xs px-3 py-1 rounded border font-medium transition disabled:opacity-50 ${u.isActive ? 'border-red-300 text-red-600 hover:bg-red-50' : 'border-green-300 text-green-600 hover:bg-green-50'}`}>
